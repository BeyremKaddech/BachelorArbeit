...
def make_discriminator():
    ...
    model = Sequential()
    if K.image_data_format() == 'channels_first':
        model.add(Convolution2D(64, (5, 5), padding='same', input_shape=(3, 64, 64)))
    else:
...
def generate_images(generator_model, output_dir, epoch):
    ...
    file."""
    test_image_stack = generator_model.predict(np.random.randn(10, 100))
    test_image_stack = (test_image_stack * 127.5) + 127.5
    test_image_stack = np.squeeze(np.round(test_image_stack).astype(np.uint8))
    tiled_output = tile_images(test_image_stack)
    tiled_output = tiled_output[:, :, ::-1]
...
#Path = glob.glob("/Users/beyrem/Documents/Studies/BachelorArbeit/Code/resized64/*.jpg")
Path = glob.glob("/var/tmp/ga94pak/Code2/faces64/*.jpg")
# X_train = inputs.read_image('/Users/beyrem/Documents/Studies/BachelorArbeit/Code/resized', 1 , 28 , 'jpg',100)
# X_train = inputs.make_batch([images], BATCH_SIZE , 4)
X_train = []
i = 0
for imagePath in Path:
    if i < 40000:
        # load the image, resize the image to be 32x32 pixels (ignoring
        print('oooooooooo', imagePath)
...
for epoch in range(300):
    ...
    for i in range(int(X_train.shape[0] // (BATCH_SIZE  * TRAINING_RATIO ))):
        ...
        for j in range(TRAINING_RATIO):
            ...
            print("Desc output: ", np.mean(discriminator_loss))
            # print("Descriminator fake: ", discriminator_output_from_generator)
            # print("Descriminator real: ", discriminator_output_from_real_samples)

        generator_loss.append(generator_model.train_on_batch(np.random.randn(BATCH_SIZE,
                                                                             100),
                                                             positive_y))
        # print("Generator loss: ", discriminator_layers_for_generator)
        print("Generator -------->", np.mean(generator_loss))

        '''
        if i % 5 == 0:
            tqdm.write('[{}/{}][{}/{}] Loss_D: {} Loss_G: {} '
                       'Loss_D_real: {} Loss_D_fake {}'.format(epoch, 300, i, int(X_train.shape[0] // (BATCH_SIZE * TRAINING_RATIO)),
                                                                ,
                                                               discriminator_layers_for_generator.data.mean(),
                                                               discriminator_output_from_real_samples.data.mean(),
                                                               discriminator_output_from_generator.data.mean()))
